...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\TheSplit Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\TheSplit Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\TheSplit manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\TheSplit.vst3\\Contents\\x86_64-win\\TheSplit.vst3",
      projectpath + "\\build-win\\TheSplit_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\TheSplit-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\TheSplit-app_x64.pdb"  
  ]
