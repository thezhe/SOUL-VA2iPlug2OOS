...
if %DEMO_ARG% == "demo" (
  echo Making TheSplit Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making TheSplit Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild TheSplit-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild TheSplit.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild TheSplit.sln /t:TheSplit-app;TheSplit-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TheSplit.aaxplugin\Contents\Win32\TheSplit.aaxplugin --out .\build-win\aax\bin\TheSplit.aaxplugin\Contents\Win32\TheSplit.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TheSplit.aaxplugin\Contents\x64\TheSplit.aaxplugin --out .\build-win\aax\bin\TheSplit.aaxplugin\Contents\x64\TheSplit.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\TheSplit.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\TheSplit.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "TheSplit Installer" ".\installer\TheSplit Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\TheSplit Installer.exe" ".\installer\TheSplit Demo Installer.exe"
  REM -del ".\installer\TheSplit Installer.exe"
  REM -)
