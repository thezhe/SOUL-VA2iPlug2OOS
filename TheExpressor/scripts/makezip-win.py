...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\TheExpressor Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\TheExpressor Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\TheExpressor manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\TheExpressor.vst3\\Contents\\x86_64-win\\TheExpressor.vst3",
      projectpath + "\\build-win\\TheExpressor_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\TheExpressor-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\TheExpressor-app_x64.pdb"  
  ]
