...
if %DEMO_ARG% == "demo" (
  echo Making TheExpressor Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making TheExpressor Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild TheExpressor-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild TheExpressor.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild TheExpressor.sln /t:TheExpressor-app;TheExpressor-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TheExpressor.aaxplugin\Contents\Win32\TheExpressor.aaxplugin --out .\build-win\aax\bin\TheExpressor.aaxplugin\Contents\Win32\TheExpressor.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TheExpressor.aaxplugin\Contents\x64\TheExpressor.aaxplugin --out .\build-win\aax\bin\TheExpressor.aaxplugin\Contents\x64\TheExpressor.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\TheExpressor.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\TheExpressor.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "TheExpressor Installer" ".\installer\TheExpressor Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\TheExpressor Installer.exe" ".\installer\TheExpressor Demo Installer.exe"
  REM -del ".\installer\TheExpressor Installer.exe"
  REM -)
