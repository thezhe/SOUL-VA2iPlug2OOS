...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\TheChorus Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\TheChorus Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\TheChorus manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\TheChorus.vst3\\Contents\\x86_64-win\\TheChorus.vst3",
      projectpath + "\\build-win\\TheChorus_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\TheChorus-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\TheChorus-app_x64.pdb"  
  ]
